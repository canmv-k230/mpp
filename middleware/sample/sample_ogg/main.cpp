...
// Process encoded data and write to Ogg file
...
static void *audio_process_thread(void *arg) {
    //Configure AI device attributes
    ...
    //Loop to get encoded data and write to file
    k_audio_stream audio_stream;
    g_audio_running = K_TRUE;
    cout << "Start capturing and encoding audio..." << endl;
...
int main(int argc, char *argv[]) {
    const char *output_filename = "/data/recorded_audio.ogg";
    ...
    strcpy(muxer_params.filename, output_filename);
    ...
    if (ret != 0) {
        cerr << "Failed to initialize Ogg muxer: " << ret << endl;
        return -1;
    }
    cout << "Ogg muxer initialized successfully. Output file: " << output_filename << endl;
    ...
    cout << "Recording audio... Press any key to stop." << endl;
    getchar();
    ...
    kd_ogg_muxer_destroy(g_ogg_muxer);
    kd_mpi_vb_exit();
    cout << "Recording finished. File saved." << endl;
    return 0;
}